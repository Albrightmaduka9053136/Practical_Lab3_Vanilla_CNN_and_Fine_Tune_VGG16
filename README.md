...
- `models_lab3/`  directory for checkpoints and saved weights (not tracked by git  see `.gitignore`).
...
I can generate a `requirements.txt` that pins exact versions from your environment if you want.
...
8. Evaluation: load best models, compute accuracy, confusion matrix, classification report (precision/recall/F1), precisionrecall curve and Average Precision (AP).
9. Error analysis: visualize misclassified test images and discuss failure modes.

## Models & training details

- **Vanilla CNN:** 3 conv blocks (3264128 filters), max pooling, flatten, Dense(128), Dropout(0.5), Dense(1, sigmoid). Optimizer: Adam (1e-4); loss: `binary_crossentropy`.
...
- Large artifacts (dataset and model weights) are intentionally not tracked by git  see `.gitignore` (the repository stores the notebook and small config files only).
- If you need the large files removed from the remote history, I can run a history-rewrite (BFG or `git filter-repo`)  this rewrites commits and requires coordination.
